...
ncap <- table(tmp$data[,1])            # ncap = 1 if no individuals captured
sites0 <- tmp$data[is.na(tmp$data[,2]),][,1] # sites where nothing was seen
ncap[as.character(sites0)] <- 0    # Fill in 0 for sites with no detections
ncap <- as.vector(ncap)            # Number of individuals detected per site

# Other data
site <- tmp$data[!is.na(tmp$data[,2]),1]   # Site ID of each observation
delta <- 0.1                       # Distance bin width for rect. approx.
midpt <- seq(delta/2, tmp$B, delta)    # Make mid-points and chop up data
dclass <- tmp$data[,5] %/% delta + 1   # Convert distance to distance category
nD <- length(midpt)                # Number of distance intervals
dclass <- dclass[!is.na(tmp$data[,2])] # Observed categorical observations
nind <- length(dclass)             # Total number of individuals detected

# Bundle and summarize data set
str( win.data <- list(nsites=tmp$nsites, nind=nind, B=tmp$B, nD=nD, midpt=midpt,
    delta=delta, ncap=ncap, habitat=tmp$habitat, wind=tmp$wind, dclass=dclass,
    site=site) )
...
out4 <- jagsUI::jags(win.data, inits, params, "model4.txt",
   n.thin=nt, n.chains=nc, n.burnin=nb, n.iter=ni)
print(out4, 2)
